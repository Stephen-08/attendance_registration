...
const Requests = () => {
  ...
  const fetchLateArrival = async () => {
    try {
      const res = await fetch("https://attendancebackend.duckdns.org/api/late-arrival-requests");
  ...
  const fetchLeaveRequests = async () => {
    try {
      const res = await fetch("https://attendancebackend.duckdns.org/api/leave-requests");
      const data = await res.json();
  ...
  const handleLateStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`https://attendancebackend.duckdns.org/api/late-arrival-requests/${id}/status`, {
        method: 'PUT',
  ...
  const handleLeaveStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`https://attendancebackend.duckdns.org/api/leave-requests/${id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: newStatus })
      });
